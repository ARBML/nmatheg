...
def aggregate_tokens(dataset, config, data_config, max_len = 128):
    ...
    token = data_config[dataset_name]['text']
    tag = data_config[dataset_name]['label']
    ...
    for split in dataset:
        ...
        for i, item in enumerate(dataset[split]):
            token, label = item[token], item[tag]
            ...
            if len(sentence) == max_len:
                ...
                sentence = []
                sent_label = []
        new_dataset[split] = Dataset.from_dict({token:sentences, tag:sent_labels}) 

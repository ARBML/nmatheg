...
def prepare_features(examples, tokenizer, data_config, model_type = 'bert', max_len = 128):
    ...
    if 'bert' in model_type:
        ...
        for i, (question, context) in enumerate(zip(examples[question_col], examples[context_col])):
            ...
            sampling_ids.append(samplings)
            input_ids.append(tokens)
            offset_mapping.append(offsets)
            sequence_ids.append(sequences)
        tokenized_examples = tokenized_examples.add_column("offset_mapping", offset_mapping)
        ...
        tokenized_examples = tokenized_examples.add_column("sampling_ids", sampling_ids)
        ...
        tokenized_examples = tokenized_examples.add_column("start_positions", end_positions)     
        return tokenized_examples